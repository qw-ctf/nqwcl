...
{
	SDL_memset (con_main.text, ' ', CON_TEXTSIZE);
	SDL_memset (con_chat.text, ' ', CON_TEXTSIZE);
}
...
void Con_Resize (console_t *con)
{
	...
	if (width < 1)			// video hasn't been initialized yet
	{
		...
		SDL_memset (con->text, ' ', CON_TEXTSIZE);
	}
	else
	{
		...
		SDL_memcpy (tbuf, con->text, CON_TEXTSIZE);
		SDL_memset (con->text, ' ', CON_TEXTSIZE);
...
void Con_Linefeed (void)
{
	...
	if (con->display == con->current)
		con->display++;
	con->current++;
	SDL_memset (&con->text[(con->current%con_totallines)*con_linewidth]
	, ' ', con_linewidth);
}

...
cvar_t          scr_conspeed = {"scr_conspeed","5000"};
...
void SCR_DrawFPS (void)
{
	...
	SDL_snprintf(st, sizeof(st), "%3d FPS", lastfps);
...
// 
	...
	{ 
		pcxname[5] = i/10 + '0'; 
		pcxname[6] = i%10 + '0'; 
		SDL_snprintf (checkname, sizeof(checkname), "%s/%s", com_gamedir, pcxname);
	...
	int rowbytes, byte *palette, bool upload)
{
	...
	SDL_memset (pcx->palette,0,sizeof(pcx->palette));
	pcx->color_planes = 1;		// chunky image
	pcx->bytes_per_line = LittleShort((short)width);
	pcx->palette_type = LittleShort(2);		// not a grey scale
	SDL_memset (pcx->filler,0,sizeof(pcx->filler));

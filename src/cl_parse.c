...
void CL_ParseDownload (void)
{
	...
	if (!cls.download)
	{
		if (SDL_strncasecmp(cls.downloadtempname,"skins/",6))
			SDL_snprintf (name, sizeof(name), "%s/%s", com_gamedir, cls.downloadtempname);
		else
			SDL_snprintf (name, sizeof(name), "qw/%s", cls.downloadtempname);
	...
	else
	{
		...
		if (strcmp(cls.downloadtempname, cls.downloadname)) {
			if (strncmp(cls.downloadtempname,"skins/",6)) {
				SDL_snprintf (oldn, sizeof(oldn), "%s/%s", com_gamedir, cls.downloadtempname);
				SDL_snprintf (newn, sizeof(newn), "%s/%s", com_gamedir, cls.downloadname);
			} else {
				SDL_snprintf (oldn, sizeof(oldn), "qw/%s", cls.downloadtempname);
				SDL_snprintf (newn, sizeof(newn), "qw/%s", cls.downloadname);
			}
			r = rename (oldn, newn);
			if (r)
...
// allow 2.2 and 2.29 demos to play
	...
	if (SDL_strcasecmp(gamedirfile, str)) {
		// save current config
